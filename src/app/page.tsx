...
export default function Page() {
  ...
  const [isCameraOn, setIsCameraOn] = useState(false); // カメラ起動有無
  let lastVideoTime = -1; // 経過時間
  let landmarks11: NormalizedLandmark | null = null; // 右肩
  let landmarks12: NormalizedLandmark | null = null; // 左肩
  let hasSentGood = false; // Good
  let hasSentBad = false; // Bad
  let leftShoulder: NormalizedLandmark | null = null; // 右肩
  let rightShoulder: NormalizedLandmark | null = null; // 左肩
  const text1 = "開始ボタンを押してください";
  const text2 = "カメラ起動中";
  const [loading, setLoading] = useState(false); // 読み込み中

  async function maybeSendGood() {
  ...
  const stopCamera = async () => {
    await fetch("http://127.0.0.1:1880/msg", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state: "noData" }),
    });
    const video = videoRef.current;
      ...
      if (video.currentTime !== lastVideoTime) {
        ...
        if (ctx) {
          ...
          if (faceResult.faceLandmarks) {
            ...
            const faceWidth =
              Math.abs(rightCheek.x - leftCheek.x) * canvas.width;
            const maskSize = faceWidth * 1.5;
